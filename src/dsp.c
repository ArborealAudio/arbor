...
static void filter_set_coeffs(Filter *f) {
    if (f->type < Filter_FirstOrderLowpass && f->reso == 0) {
    ...
    case Filter_FirstOrderHighpass: {
        const f64 fc = f->cutoff / sr;
        f->a1 = exp(-fc * TWO_PI);
    ...
    case Filter_FirstOrderHighshelf: {
    ...
    case Filter_FirstOrderLowshelf: {
